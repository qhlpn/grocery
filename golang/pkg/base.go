...
func routine() {
	...
	// 并发工具
	// 1. 同步协调  sync.WaitGroup
	// 2. 互斥锁	sync.Mutex
	var wg sync.WaitGroup
	var lock sync.Mutex
	var y func(thread string) = func(thread string) {
		for i := 0; i < 1000; i++ {
			lock.Lock() // 加锁
			fmt.Printf("%d %s\n", i, thread)
			lock.Unlock() // 解锁
	...
	// 原子操作  sync/atomic
	var t int32 = 3
	r := atomic.CompareAndSwapInt32(&t, t, 1)
	fmt.Println(r, n)
